...
const PinCode = ({
  ...
  if (curMode == PinCodeT.Modes.Enter || curMode == PinCodeT.Modes.Set) {
    const buttonStyle = StyleSheet.flatten([
      ...
      styles?.enter?.buttons,
    ]);
    return (
      <View style={[defaultStyles.mainContainer, styles?.main]}>
        <View
          style={[defaultStyles.titleContainer, styles?.enter?.titleContainer]}
        >
          <Text style={[defaultStyles.title, styles?.enter?.title]}>
            {curMode == PinCodeT.Modes.Enter
              ? curTextOptions.enter?.title
              : curTextOptions.set?.title}
          </Text>
          {curMode == PinCodeT.Modes.Enter ? (
            <>
              <Text style={[defaultStyles.subTitle, styles?.enter?.subTitle]}>
                {curTextOptions.enter?.subTitle?.replace(
                  "{{pinLength}}",
                  (curOptions.pinLength || DEFAULT.Options.pinLength).toString()
                )}
              </Text>
              {showError && (
                <Text style={defaultStyles.error}>
                  {curTextOptions.enter?.error}
                </Text>
              )}
            </>
          ) : (
            <>
              {status == PinCodeT.Statuses.Initial && (
                <Text style={[defaultStyles.subTitle, styles?.enter?.subTitle]}>
                  {curTextOptions.set?.subTitle?.replace(
                    "{{pinLength}}",
                    (
              ...
              {status == PinCodeT.Statuses.SetOnce && (
                <Text style={[defaultStyles.subTitle, styles?.enter?.subTitle]}>
                  {curTextOptions.set?.repeat}
                </Text>
              )}
              {showError && (
                <Text style={defaultStyles.error}>
                  {curTextOptions.set?.error}
                </Text>
              )}
            </>
          )}
        ...
        >
          ...
          <View style={defaultStyles.pinNumberRow}>
            <View
              style={[defaultStyles.button, { width: 60, height: 60 }]}
            ></View>
            <PinButton
              value={"0"}
              disabled={buttonsDisabled}
            ...
            <PinButton
              ...
              backSpaceText={curTextOptions?.enter?.backSpace}
            ...
            >
              <Text
                style={[
                  ...
                  styles?.enter?.footerText,
                ]}
              >
                {curTextOptions.enter?.footerText}
              </Text>
            </TouchableOpacity>
          )}
    ...
    return (
      <View style={[defaultStyles.mainContainer, styles?.main]}>
        <View
          style={[defaultStyles.titleContainer, styles?.locked?.titleContainer]}
        >
          <Text style={[defaultStyles.title, styles?.locked?.title]}>
            {curTextOptions.locked?.title}
          </Text>
          <Text style={[defaultStyles.subTitle, styles?.locked?.subTitle]}>
            {curTextOptions.locked?.subTitle
              ?.replace(
                "{{maxAttempt}}",
                (curOptions.maxAttempt || DEFAULT.Options.maxAttempt).toString()
              )
              .replace(
                "{{lockDuration}}",
                millisToMinutesAndSeconds(
                  curOptions.lockDuration || DEFAULT.Options.lockDuration
                )
              )}
          </Text>
        </View>
        <View style={defaultStyles.pinContainer}>
          {options?.lockIcon ? (
            ...
            <Text style={styles?.locked?.locked}>
              {curTextOptions.locked?.lockedText}
            </Text>
          )}
        </View>
        <View style={defaultStyles.buttonContainer}>
          <Clock
            style={styles?.locked?.clockContainer}
            textStyle={styles?.locked?.clockText}
    ...
    return (
      <View style={[defaultStyles.mainContainer, styles?.main]}>
        <View
          style={[defaultStyles.titleContainer, styles?.reset?.titleContainer]}
        >
          <Text style={[defaultStyles.title, styles?.reset?.title]}>
            {curTextOptions.reset?.title}
          </Text>
          <Text style={[defaultStyles.subTitle, styles?.reset?.subTitle]}>
            {curTextOptions.reset?.subTitle}
          </Text>
        </View>
        <View style={defaultStyles.buttonContainer}>
          {status == PinCodeT.Statuses.Initial && (
            <>
              <TouchableOpacity
                onPress={() => setStatus(PinCodeT.Statuses.ResetPrompted)}
              >
                <Text style={[defaultStyles.confirm, styles?.reset?.buttons]}>
                  {curTextOptions.reset?.resetButton}
                </Text>
              </TouchableOpacity>
            </>
          )}
          {status == PinCodeT.Statuses.ResetPrompted && (
            <>
              <Text style={{ color: "white", marginBottom: 20 }}>
                {curTextOptions.reset?.confirm}
              </Text>
              <TouchableOpacity onPress={onDeletePIN}>
                <Text style={[defaultStyles.confirm, styles?.reset?.buttons]}>
                  {curTextOptions.reset?.confirmButton}
                </Text>
              </TouchableOpacity>
            </>
          )}
          <TouchableOpacity onPress={() => switchMode(PinCodeT.Modes.Enter)}>
            <Text
              style={[
                ...
                styles?.reset?.buttons,
              ]}
            >
              {curTextOptions.reset?.backButton}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
